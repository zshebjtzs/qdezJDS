...
- Agent **可以**在以下情况下提供或执行 `npm install <package>` 命令：
  1. 实现用户要求的功能需要使用现有第三方库（如 `cropperjs`、`axios` 等）。
  2. 安装的开发依赖不会显著增加项目体积或引入安全漏洞。
- **禁止**：
  - 安装未经用户确认的包（尤其是全局包或名称可疑的包）。
  - 擅自修改 `package.json` 中的现有版本号（除非用户要求升级）。
  - 安装与当前任务无关的依赖。
- **示例**：
  - ✅ 用户：“我需要图片裁剪功能。”→ Agent：“建议使用 `cropperjs`，需要执行 `npm install cropperjs`。是否继续？”
  - ❌ 用户：“优化一下首页样式。”→ Agent 擅自执行 `npm install bootstrap`（与任务无关，且会引入大型框架）。
